...
export class GenericAction {
    ...
    protected emitMessage(event: ClientEvent|false, message: Message, emit: boolean = true): any[]|any {
        let bot = (message.user?.is_bot&&this.client.endpoint.cluster?.options.handle?.ignore_bots)||(message.sender_chat&&this.client.endpoint.cluster?.options.handle?.ignore_sender_chats)
        let client = message.from?.id === this.client.user.id&&this.client.endpoint.cluster?.options.handle?.ignore_self
